...
refuses to run under MTPLX_NAX_VERIFY), every lane here stays on the
``nn.QuantizedLinear.__call__`` surface — the fused hub module IS a
QuantizedLinear and the member fallback calls the class ``__call__`` — so the
NAX verify patch sees both lanes and the two features compose. packed_concats
itself is mutually exclusive with this lane (see the refusal below); this
module fuses a superset of its groups with zero-copy members.
"""
...
class _FusionHub:
    ...
    def part(self, x: mx.array, index: int) -> mx.array:
        ...
        if self._outs is None or self._key is not x:
            ...
            self._outs = None
            self._served = 0
            # Contiguous copies: stock projections emit contiguous tensors, and
            # a strided split view can route a downstream kernel (norm/rope/
...
def configure_fused_projections(model: Any | None = None) -> dict[str, Any]:
    ...
    if packed_concats_enabled():
        # One fusion lane at a time: packed_concats fuses the same q|k|v and
        # gate|up groups at the forward level, so stacking both would pack the
        # member views into a second (duplicated) payload and confound any A/B.
        _STATS["enabled"] = False
        _note_skip("all", "MTPLX_PACKED_PROJ_CONCATS is on; refusing to stack fusion lanes")
        print(
            "[proj-fusion] refused: MTPLX_PACKED_PROJ_CONCATS is also set — "
            "unset one fusion lane",
            file=sys.stderr,
            flush=True,
        )
        return fused_projection_stats()
